 ...
 * @internal
 */
function serializeValue(value, key, dateFormats = {}, codecEncoders = new Map()) {
 ...
 * @internal
 */
export function extractParamValues(searchParams, options = {}) {
    const { numberFields = new Set(), arrayFields = new Set(), dateFields = new Set(), codecFields = new Set(), } = options;
    const params = {};
 ...
 * @internal
 */
function extractZodCodecEncoder(fieldSchema) {
    // Check if this looks like a Zod schema with def property
    const zodLike = fieldSchema;
    if (!zodLike.def)
        return undefined;
...
function extractSchemaInfo(schema) {
    ...
    const codecEncoders = new Map();
    const codecFields = new Set();
 ...
 * @internal
 */
function extractSelectiveParamValues(searchParams, schemaKeys, options = {}) {
    const { numberFields = new Set(), arrayFields = new Set(), dateFields = new Set(), codecFields = new Set(), } = options;
    const params = {};
...
class SearchParams {
    ...
    #handleCompressedUpdate(fullParamsObject) {
        try {
            ...
            const serializedObject = { ...fullParamsObject };
...
export function createSearchParamsSchema(schema) {
    return {
        ...
        __dateFormats: Object.entries(schema).reduce((acc, [key, config]) => {
...
export function useSearchParams(schema, options = {}) {
    ...
    $effect(() => {
        if (!isMounted.current || building || options.updateURL === false)
            return;
